...
bool NvofBridge::Init(ID3D12Device* device, ID3D12Resource* liveSource, UINT width, UINT height, Log log) {
    Shutdown(); log_=std::move(log); device_=device; source_=liveSource; width_=width; height_=height;
    ...
    if(pixels<=uint64_t(2560)*1440 && supports(1)) grid_=1; else if(supports(2)) grid_=2; else if(supports(1)) grid_=1; else if(supports(4)) grid_=4; else {Shutdown();return false;}
    ...
    init.perfLevel=(pixels<=uint64_t(2560)*1440)?NV_OF_PERF_LEVEL_MEDIUM:NV_OF_PERF_LEVEL_FAST;
    ...
    init.predDirection=backward_?NV_OF_PRED_DIRECTION_BOTH:NV_OF_PRED_DIRECTION_FORWARD;
    ...
    if(st!=NV_OF_SUCCESS&&backward_){
        api_->fn.nvOFDestroy(h); session_=nullptr; backward_.Reset(); backwardCost_.Reset();
        st=api_->fn.nvCreateOpticalFlowD3D12(device,reinterpret_cast<NvOFHandle*>(&session_));
        if(st==NV_OF_SUCCESS&&session_){ h=static_cast<NvOFHandle>(session_); init.predDirection=NV_OF_PRED_DIRECTION_FORWARD; st=api_->fn.nvOFInit(h,&init); }
    }
    if(st!=NV_OF_SUCCESS){ if(log_)log_(std::wstring(L"NVOFA init failed: ")+NvofStatus(st)); Shutdown(); return false; }

    if(!Register(source_.Get(),&sourceHandle_)||!Register(previous_.Get(),&previousHandle_)||!Register(flow_.Get(),&flowHandle_)||
       (cost_&&!Register(cost_.Get(),&costHandle_))||(backward_&&!Register(backward_.Get(),&backwardHandle_))||
       (backward_&&backwardCost_&&!Register(backwardCost_.Get(),&backwardCostHandle_))){ if(log_)log_(L"NVOFA resource registration failed."); Shutdown(); return false; }

    ready_=true; hasPrevious_=false; flowValid_=false; disableTemporalOnce_=true;
    if(log_)log_(L"NVOFA direct-source motion: "+std::to_wstring(grid_)+L"x"+std::to_wstring(grid_)+L" grid, "+
        ((pixels<=uint64_t(2560)*1440)?std::wstring(L"MEDIUM"):std::wstring(L"FAST"))+L" preset, no pre-flow graphics copy, backward consistency="+
        (backward_?std::wstring(L"on"):std::wstring(L"off"))+L".");
    return true;
}

bool NvofBridge::SubmitFrame(ID3D12Fence* captureFence,uint64_t captureValue){
    if(!ready_||!captureFence){flowValid_=false;return false;}
    ...
    NV_OF_EXECUTE_INPUT_PARAMS_D3D12 in{}; in.inputFrame=static_cast<NvOFGPUBufferHandle>(sourceHandle_); in.referenceFrame=static_cast<NvOFGPUBufferHandle>(previousHandle_);
    ...
    out.bwdOutputBuffer=static_cast<NvOFGPUBufferHandle>(backwardHandle_); out.bwdOutputCostBuffer=static_cast<NvOFGPUBufferHandle>(backwardCostHandle_); out.fencePoint=&done;
...
void NvofBridge::UnregisterAll(){
    if(!api_||!api_->fn.nvOFUnregisterResourceD3D12)return; void** handles[]={&sourceHandle_,&previousHandle_,&flowHandle_,&costHandle_,&backwardHandle_,&backwardCostHandle_};
...
void NvofBridge::Shutdown(){
    ready_=false;hasPrevious_=false;flowValid_=false;UnregisterAll();source_.Reset();previous_.Reset();flow_.Reset();cost_.Reset();backward_.Reset();backwardCost_.Reset();
