...
bool NvofBridge::Init(ID3D12Device* device, ID3D12Resource* liveSource, UINT width, UINT height, Log log) {
    ...
    if(!library_){ if(log_)log_(L"NVOFA unavailable: nvofapi64.dll not found; original motion fallback active."); return false; }
    ...
    init.predDirection=NV_OF_PRED_DIRECTION_FORWARD; init.enableGlobalFlow=NV_OF_FALSE;
...
bool NvofBridge::SubmitFrame(ID3D12Fence* captureFence,uint64_t captureValue){
    ...
    NV_OF_EXECUTE_OUTPUT_PARAMS_D3D12 out{}; out.outputBuffer=static_cast<NvOFGPUBufferHandle>(flowHandle_); out.outputCostBuffer=static_cast<NvOFGPUBufferHandle>(costHandle_); out.fencePoint=&done;
    auto st=api_->fn.nvOFExecuteD3D12(static_cast<NvOFHandle>(session_),&in,&out); flowValid_=st==NV_OF_SUCCESS;
    if(!flowValid_&&log_)log_(std::wstring(L"NVOFA execute failed: ")+NvofStatus(st)+L"; original motion fallback used for this frame.");
    return flowValid_;
}
...
void NvofBridge::BeginRead(ID3D12GraphicsCommandList* list){
    ID3D12Resource* r[]={flow_.Get(),cost_.Get()};
...
void NvofBridge::RecordHistoryCopy(ID3D12GraphicsCommandList* list,ID3D12Resource* source,D3D12_RESOURCE_STATES sourceState){
    ...
    b[1].Transition={previous_.Get(),D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES,D3D12_RESOURCE_STATE_COMMON,D3D12_RESOURCE_STATE_COPY_DEST};
...
void NvofBridge::UnregisterAll(){
    ...
    void** handles[]={&sourceHandle_,&previousHandle_,&flowHandle_,&costHandle_};
...
void NvofBridge::Shutdown(){
    ready_=false;hasPrevious_=false;flowValid_=false;UnregisterAll();source_.Reset();previous_.Reset();flow_.Reset();cost_.Reset();
