...
public:
    ...
    ~NvofBridge();
    bool Init(ID3D12Device* device, ID3D12Resource* const* liveSources, UINT sourceCount, UINT width, UINT height, Log log);
    void Shutdown();
...
private:
    ...
    uint64_t ofaValue_{};
    std::vector<Microsoft::WRL::ComPtr<ID3D12Resource>> sources_;
    Microsoft::WRL::ComPtr<ID3D12Resource> previous_, flow_, cost_, backward_, backwardCost_;
    std::vector<void*> sourceHandles_;
